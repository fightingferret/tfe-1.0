  ...
  for( i = -1; i < MAX_TABLE; i++ ) {
    ...
    for( j = 0; j < table_max[table]; j++ )
	 ...
	 case VAR_DICE :
         ...
         case VAR_POS :
          if( (*pntr = (char *) fread_number( fp ) ) < 0 || ((int) *pntr) >= MAX_POSITION ) 
...
void do_tset( char_data* ch, char* argument )
{
  ...
  switch( entry_type( i,k ) ) {
    ...
    case VAR_CENT :
      ...
      *pntr = (void *) n;
      pntr = (void **) table_pntr( i,0,j );
      sprintf( tmp, "%s on %s %s set to %.2f.\r\n", table_field[i][k], table_name[i], (char*) *pntr, (float) n/100 );
      break;
    ...
    case VAR_SKILL :
      ...
      *pntr = (void *) n;
      pntr = (void **) table_pntr( i,0,j );
      sprintf( tmp, "%s on %s %s set to %s.\r\n", table_field[i][k], table_name[i], (char*) *pntr, n == -1 ? "none" : skill_table[n].name );
      break;
    ...
    case VAR_SA :
      ...
      *pntr = (void *) n;
      pntr = (void **) table_pntr( i,0,j );
      sprintf( tmp, "%s on %s %s set to %s.\r\n", table_field[i][k], table_name[i], (char*) *pntr, spell_act_table[n].name );
      break;
    ...
    case VAR_LEECH :
    case VAR_DICE : {
      class dice_field entry = { table_field[i][k], LEVEL_MOB, (int*) pntr };
      pntr = (void **) table_pntr( i,0,j );
