...
const char* whistle_msg = 
 ...
 daemon.\r\nHe stamps up to you and mutters a few strange words, then in\
 an\r\nuncharacteristic gesture smiles happily and waves to you.  Finally\
    ...
    for( i = 0; i < *list1; i++ ) {
      ...
      for( j = 0; j < room->exits; j++ ) {
        ...
        if( to_room->distance == 0 
          ...
          bool up   = (exit_direction( to_room, DIR_UP ) == NULL ? FALSE : TRUE);
          bool down = (exit_direction( to_room, DIR_DOWN ) == NULL ? FALSE : TRUE);
          plot( map, length, width, pos,
            up ? ( down ? 'X' : '>' ) : ( down ? '<' : 'O' ),
            top, bottom );
